...
function render() {
  ...
  var savedScrollTop = lyricsEl ? lyricsEl.scrollTop : 0;
  ...
  var newLyricsEl = document.getElementById('lyrics');
  if (newLyricsEl) newLyricsEl.scrollTop = savedScrollTop;
}
...
function renderLibraryOverlay() {
  ...
  return '<div class="overlay" onclick="event.stopPropagation()">' +
    ...
    '<div style="padding:8px 8px 0"><input type="text" id="lib-search" placeholder="Search songs..." oninput="filterLibrary(this.value)" style="width:100%;padding:8px 12px;border-radius:6px;border:1px solid #334155;background:#1e293b;color:#e2e8f0;font-size:14px;outline:none;"></div>' +
    '<div class="overlay-list"><div class="overlay-grid" id="lib-grid">' + items + '</div></div></div>';
}
...
function filterLibrary(query) {
  ...
  for (var i = 0; i < LIBRARY.length; i++) {
    var s = LIBRARY[i];
    if (q && (s.title || '').toLowerCase().indexOf(q) === -1 && (s.artist || '').toLowerCase().indexOf(q) === -1) continue;
    items += '<button class="lib-card" onclick="pickLibrarySong(' + i + ')">' +
      '<div class="overlay-title" dir="auto">' + esc(s.title) + '</div>' +
      (s.artist ? '<div class="overlay-artist">' + esc(s.artist) + '</div>' : '') + '</button>';
  }
  if (items === '') items = '<p style="text-align:center;color:#64748b;padding:32px">No matching songs</p>';
      ...
      try {
        ...
        dgProcessor.onaudioprocess = function(e) {
          ...
          if (resampleBuf) {
            ...
            samples = resampleBuf; len = outLen;
          }
          for (var i = 0; i < len; i++) {
...
// Layer 2: Canvas-based video fallback (keeps screen on via animated stream)
var noSleepVideo = null;
var noSleepCanvas = null;
var noSleepEnabled = false;
var noSleepAnimFrame = null;
var noSleepHue = 0;
...
function enableNoSleep() {
  ...
  if (!noSleepCanvas) {
    ...
    var ctx = noSleepCanvas.getContext('2d');

    // Animate changing colors so the captured stream has real frames
    function paintLoop() {
      noSleepHue = (noSleepHue + 1) % 360;
      ctx.fillStyle = 'hsl(' + noSleepHue + ',50%,50%)';
      ctx.fillRect(0, 0, 2, 2);
      noSleepAnimFrame = requestAnimationFrame(paintLoop);
    }
    paintLoop();

    // Capture the animated canvas at 30fps
    var stream = noSleepCanvas.captureStream(30);
